...
export const updateEmployeeWithPhoto = async (id: number, employeeData: any, photoFile?: File) => {
  ...
  // Always add all employee data to FormData, converting as needed
  Object.keys(employeeData).forEach(key => {
    ...
    if (["overall_percent"].includes(key)) {
      value = value === "" ? null : parseFloat(value) || 0.0;
    }
    // For empty string or null, send as empty string (or null if backend expects)
...
export const updateEmployeeTrainingModulesBulk = async (employeeId: number, updates: Array<{module_id: number, status: string, completed_date?: string | null}>) => {
  const csrftoken = getCookie('csrftoken');
  const response = await fetch(`${API_BASE_URL}employees/${employeeId}/update_training_modules/`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-CSRFToken': csrftoken || '',
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({ error: 'Failed to update training modules' }));
    throw new Error(errorData.error || 'Failed to update training modules');
  }
  return response.json();
}; 